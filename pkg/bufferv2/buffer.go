...
// NewWithView creates a new Buffer initialized with given view. This function
// takes ownership of v.
func NewWithView(v *View) Buffer {
	b := Buffer{
		size: int64(v.ReadSize()),
	}
	if b.size == 0 {
		return b
	}
...
func (b *Buffer) ReadAt(p []byte, offset int64) (int, error) {
	...
	for v := b.data.Front(); v != nil && done < int64(len(p)); v = v.Next() {
		...
		if sz := v.ReadSize(); sz <= needToSkip {
			skipped += int64(sz)
			continue
		}

		// Actually read data.
		n := copy(p[done:], v.ReadSlice()[needToSkip:])
...
// Truncate truncates the Buffer to the given bytes.
//
...
// Grow grows the given Buffer to the number of bytes, which will be appended.
// If zero is true, all these bytes will be zero. If zero is false, then this is
// the caller's responsibility.
//
// Precondition: length must be >= 0.
func (b *Buffer) Grow(length int64, zero bool) {
	if length < 0 {
		panic("negative length provided")
	}
	for b.size < length {
...
func (b *Buffer) Prepend(src *View) {
	...
	if !v.sharesChunk() {
		avail := v.read
		vStart := 0
		srcStart := src.ReadSize() - avail
		if avail > src.ReadSize() {
			vStart = avail - src.ReadSize()
			srcStart = 0
		}
		// Save the write index and restore it after.
		old := v.write
		v.read = vStart
		n := v.WriteAt(0, src.ReadSlice()[srcStart:])
		b.size += int64(n)
		v.write = old
		src.write = srcStart
...
func (b *Buffer) PullUp(offset, length int) (*View, bool) {
	...
	// Merge the buffers.
	merged := NewView(totLen)
	off := 0
	for n := v; n != nil && off < totLen; {
		merged.WriteAt(off, n.ReadSlice())
		off += n.ReadSize()
...
func (b *Buffer) WriteFromReader(r io.Reader, count int64) (int64, error) {
	...
	for done < count {
		...
		n, err = r.Read(view.WriteSlice()[:sz])
		view.WriteMove(n)
		done += int64(n)
		b.size += int64(n)
		if err == io.EOF {
...
func (b *Buffer) ReadToWriter(w io.Writer, count int64) (int64, error) {
	...
	for view := b.data.Front(); view != nil && done < count; view = view.Next() {
		...
		if sz < minBatch && left >= int64(minBatch) && (b.size-done) >= int64(minBatch) {
			tmp := NewView(minBatch)
			n, err = b.ReadAt(tmp.WriteSlice()[:minBatch], done)
			tmp.WriteMove(n)
			w.Write(tmp.ReadSlice())
			tmp.Release()
			done += int64(n)
		...
		n, err = w.Write(view.ReadSlice()[offset : offset+sz])
		done += int64(n)
		if err != nil {
			break
		}
...
func (b *Buffer) Readers() []bytes.Reader {
	readers := make([]bytes.Reader, 0, b.data.Len())
	for buf := b.data.Front(); buf != nil; buf = buf.Next() {
