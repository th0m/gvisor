		...
		for fillName, fn := range fillFuncs {
			t.Run(fillName+"/"+tc.name, func(t *testing.T) {
				...
				buf.Apply(func(v *View) {
					appliedOut = append(appliedOut, v.ReadSlice()...)
				})
...
func TestBufferPullUp(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if gotOk != wantOk || !bytes.Equal(got.ReadSlice(), want) {
				t.Errorf("v.PullUp(%d, %d) = %q, %t; %q, %t", tc.offset, tc.length, got.ReadSlice(), gotOk, want, wantOk)
			}

			var gotLengths []int
			for v := b.data.Front(); v != nil; v = v.Next() {
				gotLengths = append(gotLengths, v.ReadSize())
			}
...
func TestBufferClone(t *testing.T) {
	...
	b := NewWithData(bytes.Repeat([]byte{originalSize}, originalSize))
	clonedB := b.Clone()
